...
namespace VPetLLM.Utils.Network
{
    /// <summary>
    ...
    ///
    /// 探测打的是与业务接口同级的 <c>…/health</c>，该路径不存在时退回网关根部的 <c>/health</c>。
    ///
    /// 判据只有一条：**能不能在超时内拿到 HTTP 响应**。为什么不再看状态码——官方网关对
    /// 没有签名的探测一律拒绝，并且把拒绝伪装成随机错误码（500/502/503/504/429 里挑一个），
    /// 裸探测永远拿不到 2xx；而真正下线的服务靠连不上/超时体现，活着的服务器报错响应很快，
    /// 拦下来也省不了时间。拿状态码当"服务挂了"的证据，等于让预检掷骰子。
    ///
    /// 安全默认：只有在**连不上/超时**时才拦截请求。探测本身出意外、或两个候选路径
    /// 都没有 /health 时一律放行，绝不能因为预检机制本身把可用的服务判死。
    /// </summary>
    public static class FreeServiceHealthCheck
    {
            ...
            HttpClient client, string probeUrl, Action<string>? log)
        {
            try
            {
                ...
                // 拿到响应就说明服务器在线。4xx 多为鉴权拒绝，5xx 也可能是网关在鉴权层
                // 用随机错误码掩盖拒绝——裸探测本来就不带凭证，永远拿不到 2xx。
                // 服务是否真能干活，交给带凭证的真实请求去判定。
