...
namespace VPetLLM.Utils
{
    public static class ErrorMessageHelper
    {
        ...
        public static string GetFriendlyExceptionError(Exception ex, Setting settings, string providerName = "API")
        {
            ...
            return ex switch
            {
                System.Net.Http.HttpRequestException httpEx => GetHttpRequestExceptionMessage(httpEx, settings, providerName),
                TaskCanceledException => GetLocalizedErrorWithProvider("API.Error.Timeout", langCode, providerName),
                System.Net.Sockets.SocketException => GetLocalizedErrorWithProvider("API.Error.ConnectionRefused", langCode, providerName),
                System.IO.IOException => LanguageHelper.GetError("API.Error.NetworkError", langCode),
        ...
        private static string GetHttpRequestExceptionMessage(System.Net.Http.HttpRequestException ex, Setting settings, string providerName)
        {
            var langCode = settings?.Language ?? "zh-hans";
            ...
            if (message.Contains("name resolution") || message.Contains("dns") || message.Contains("主机名"))
