...
namespace VPetLLM.Validation
{
    ...
    public static class ParameterValidator
    {
        ...
        /// <summary>
        /// Validate BubbleDisplayRequest object
        /// </summary>
        /// <param name="request">Request to validate</param>
        /// <returns>Validation result</returns>
        public static ValidationResult ValidateRequest(BubbleDisplayRequest request)
        {
            try
            {
                if (request == null)
                ...
                Logger.Log($"ParameterValidator: Validating BubbleDisplayRequest - ID: {request.RequestId}");

                // Validate core parameters
                var coreResult = ValidateAll(request.Text, request.AnimationName, request.Force);
                ...
                var result = new ValidationResult
                {
                    ...
                    FieldErrors = new Dictionary<string, string>(coreResult.FieldErrors),
                    Warnings = coreResult.Warnings?.ToArray() ?? new string[0]
                };

                // Validate request ID
                if (string.IsNullOrEmpty(request.RequestId))
                {
                    result.AddFieldError("requestId", "Request ID cannot be null or empty");
                }

                // Validate request time
                if (request.RequestTime == default(DateTime))
                {
                    result.AddWarning("Request time is not set");
                }
                else if (request.RequestTime > DateTime.Now.AddMinutes(1))
                {
                    result.AddWarning("Request time is in the future");
                }

                // Validate priority
                if (request.Priority < 0)
                {
                    result.AddWarning("Request priority is negative");
                }
                ...
                if (request.TTSOptions != null)
                {
                    ...
                    {
                        result.AddFieldError("ttsOptions", ttsResult.ErrorMessage);
                    }
                    if (ttsResult.Warnings?.Length > 0)
                    {
                        foreach (var warning in ttsResult.Warnings)
                        {
                            result.AddWarning($"TTS: {warning}");
                        }
                    }
                }

                Logger.Log($"ParameterValidator: BubbleDisplayRequest validation result - IsValid: {result.IsValid}");
                return result;
            }
            catch (Exception ex)
            {
                Logger.Log($"ParameterValidator: Error validating BubbleDisplayRequest: {ex.Message}");
                return ValidationResult.Failure($"Error validating request: {ex.Message}");
            }
        }
