...
namespace VPetLLM.Core.Integration.UnifiedTTS.Management
{
    /// <summary>
    /// 统一的 TTS 调度器实现
    ...
    {
        private readonly UnifiedTTSConfigManager _configManager;
        ...
        private ModelsServiceStatus _serviceStatus;
        private ModelsTTSRequest _currentRequest;
        private CancellationTokenSource _currentCancellationTokenSource;
        private bool _isInitialized = false;

        public TTSDispatcher(
            UnifiedTTSConfigManager configManager,
        ...
        public async Task<TTSResponse> ProcessRequestAsync(ModelsTTSRequest request)
        {
            if (request is null)
            {
        ...
        private async Task<TTSResponse> ProcessRequestWithRetryAsync(ModelsTTSRequest request, TTSConfiguration configuration, ErrorContext context)
        {
        ...
        public async Task<ModelsServiceStatus> GetServiceStatusAsync()
        {
            try
            {
                ...
                lock (_dispatcherLock)
                {
                    ...
                    status.ActiveRequests = _currentRequest is not null ? 1 : 0;
                }
        ...
        public ValidationResult ValidateRequest(ModelsTTSRequest request)
        {
            try
            {
                if (request is null)
        ...
        public void CancelCurrentRequest()
        {
            try
            {
                ...
                lock (_dispatcherLock)
                {
                    if (_currentRequest is not null && _currentCancellationTokenSource is not null)
                    {
        ...
        public bool IsActive()
        {
            lock (_dispatcherLock)
            {
                return _currentRequest is not null && _serviceStatus.ActiveRequests > 0;
            }
        }
        ...
        {
            _serviceStatus = new ModelsServiceStatus
            {
                IsAvailable = true,
        ...
        private void OnConfigurationChanged(object sender, UnifiedTTSConfigChangedEventArgs e)
        {
            try
            {
                ...
                lock (_dispatcherLock)
                {
                    if (e.NewConfiguration is not null)
                    {
            ...
            try
            {
                ...
                switch (newConfiguration.Type)
                {
                    case TTSType.External:
                        if (newConfiguration.ExternalSettings is not null)
                        {
                    ...
                    case TTSType.BuiltIn:
                        if (newConfiguration.BuiltInSettings is not null)
                        {
        ...
        /// <returns>TTS 响应</returns>
        private async Task<TTSResponse> RouteRequestAsync(ModelsTTSRequest request, TTSConfiguration configuration, CancellationToken cancellationToken = default)
        {
            try
            {
        ...
        /// <param name="cancellationToken">取消令牌</param>
        /// <returns>TTS 响应</returns>
        private async Task<TTSResponse> ProcessWithTimeoutAsync(ITTSAdapter adapter, ModelsTTSRequest request, CancellationToken cancellationToken)
        {
            try
            {
                // 组合超时令牌和外部取消令牌
