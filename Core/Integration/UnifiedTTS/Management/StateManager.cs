...
namespace VPetLLM.Core.Integration.UnifiedTTS.Management
{
    ...
    public interface ITTSStateManager
    {
        ...
        Task UpdateServiceStatusAsync(ModelsServiceStatus status);
    ...
    {
        private readonly ModelsServiceStatus _serviceStatus;
        ...
        /// 更新服务状态
        /// </summary>
        public async Task UpdateServiceStatusAsync(ModelsServiceStatus status)
        {
            try
            {
                if (status is null)
        ...
        public async Task RegisterActiveRequestAsync(ActiveRequestInfo requestInfo)
        {
            try
            {
                if (requestInfo is null || string.IsNullOrEmpty(requestInfo.RequestId))
                    return;
