...
namespace VPetLLM.Core.Integration.UnifiedTTS.Adapters
{
    /// <summary>
    /// 内置 TTS 适配器
    ...
    public class BuiltInTTSAdapter : IBuiltInTTSAdapter
    {
        ...
        public async Task<TTSResponse> ProcessAsync(ModelsTTSRequest request)
        {
            if (request is null)
            {
        ...
        {
            if (_vpetAPI is null && vpetAPI is not null)
            {
